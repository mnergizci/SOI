...
def get_sf_array(path_to_slcdir, f0=5405000500, burst_interval = 2.758277):
    ...
    for key, value in ext_burst_win_r1_dict.items():
        parts = key.split('_')
        sw_number, burst_number = int(parts[0][-1]), int(parts[-1])  # Correctly extract sw number and burst number
        # print(sw_number, burst_number)
        ...
        if sw_number == 1 or sw_number == 2:  # For subswath1's and subswath2's in r1_dic
            # Adjust the key format to indicate it's calculated from sw_number to sw_number+1
            new_key_format = f'SW{sw_number}-{sw_number + 1}_{burst_number}' ##like SW1-2_1 .... SW2-3_25
            ...
            if next_sw_key_r0 in ext_burst_win_r0_dict:
                #print(key, next_sw_key_r0)
                r0 = ext_burst_win_r0_dict[next_sw_key_r0]  # Matching entry in r0_dic
                r0_ml=(r0-rps/2)+rps_ml/2
                r1= ext_burst_win_r1_dict[key] # or r1 = value  # Current value in r1_dic 
                r1_ml=((r1-rps/2)+rps_ml/2)
                #nr=int(round(r1-r0)/rps)+1
                nr=int(np.ceil(r1_ml-r0_ml)/rps_ml)
    ...
    def calculate_kr_and_kt(r_so, afmrate_srdelay, afmrate_poly, ka):
        """
        Calculate kr and kt values based on given parameters.
    ...
    def process_r_so_results(r_so_results, sw_numbers, afmrate_srdelay_dict, afmrate_poly_dict, ka_dict):
        """
        Process the calculated range sequence overlaps to determine kr and kt values for each burst.
    
        Args:
            r_so_results (dict): Dictionary containing range sequences for subswath overlaps.
            sw_numbers (list): List of subswath numbers being processed.
    ...
    def calculate_azi_line(path_to_slcdir, ext_burst_win_a0_dict, ext_burst_win_a1_dict, sw_numbers, azps, kr_dict, kt_dict):
        ...
        for i in range(len(sw_numbers)-1):
            ...
            tazi_ml=tazi*4 ##multilooking 4 in azimuth
             ...
             # Extract keys for both subswaths
            sw1_keys = [key for key in burst_start_time_dict.keys() if key.startswith(f"SW{sw1}_")]
            sw2_keys = [key for key in burst_start_time_dict.keys() if key.startswith(f"SW{sw2}_")]
            ...
            for sw1_key in sw1_keys:
                for sw2_key in sw2_keys:
                    ...
                    #print(sw1_key, ext_burst_win_a1_dict[sw1_key])
                    if ext_burst_win_a1_dict[sw1_key]>ext_burst_win_a0_dict[sw2_key] and ext_burst_win_a1_dict[sw2_key]>ext_burst_win_a0_dict[sw1_key]: ### check burst overlaps: bwr and fwr!
                        ...
                        if ext_burst_win_a0_dict[sw2_key] > ext_burst_win_a0_dict[sw1_key]: ### situation of forward SOI
                            ...
                            # print(len(ti_ml - t1_sw1_ml))
                            # print(kt_dict[sw1_key])
    ...
    ##let's obtain both subswath values....
    sw_numbers = ['1', '2']  # ['2', '3'] # Process calculations for both SW1 and SW2, I need to update it for SW2 and SW3?
    ...
    sw_numbers = ['2', '3']  # ['2', '3'] # Process calculations for both SW1 and SW2, I need to update it for SW2 and SW3?
    kr_dict, kt_dict= process_r_so_results(r_so_results, sw_numbers, afmrate_srdelay_dict, afmrate_poly_dict, ka_dict)
